...
if (isset($search_value)) {
  ...
  if (is_string($search_value)) {
    $archive_sql = $conn->prepare("SELECT * FROM water_installation WHERE step = 'Complete' AND customer_name LIKE '%$search_value%' LIMIT 16");
  // If the search values is numeric for the searching tracking number.
  } if (is_numeric($search_value)) {
    $archive_sql = $conn->prepare("SELECT * FROM water_installation WHERE step = 'Complete' AND tracking_number LIKE '%$search_value%' LIMIT 8");
  }
  ...
  $all_sql = $conn->prepare("SELECT * FROM water_installation WHERE step = 'Complete'");
  $all_sql->execute();

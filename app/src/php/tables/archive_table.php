...
if (isset($search_value)) {
  ...
  if (is_string($search_value)) {
    $archive_sql = $conn->prepare("SELECT * FROM water_installation WHERE step = 'Completed' AND time_updated < NOW() - INTERVAL 2 DAY AND (first_name LIKE '%$search_value%') OR (last_name LIKE '%$search_value%') OR (middle_name LIKE '%$search_value%') ORDER BY time_updated DESC");

...
if (isset($search_value)) {
    ...
    if (is_string($search_value)) {
      $phase_2_step_4_complete_sql = $conn->prepare("SELECT * FROM water_installation WHERE step = 'Phase-2-Step-4-Complete' AND (CONCAT(first_name, ' ', last_name, ' ', middle_name) LIKE '%$search_value%') ORDER BY time_updated DESC");
    ...
    if(!$phase_2_step_4_complete_sql) {

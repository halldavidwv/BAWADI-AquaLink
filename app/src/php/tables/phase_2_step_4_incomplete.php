...
if (isset($search_value)) {
  ...
  if (is_string($search_value)) {
    $phase_2_step_4_incomplete_sql = $conn->prepare("SELECT * FROM water_installation WHERE step = 'Phase-2-Step-4-Incomplete' AND (first_name LIKE '%$search_value%') OR (last_name LIKE '%$search_value%') OR (middle_name LIKE '%$search_value%') ORDER BY time_updated DESC");

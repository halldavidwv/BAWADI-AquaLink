...
if (isset($search_value)) {
    ...
    if (is_string($search_value)) {
      $completed_sql = $conn->prepare("SELECT * FROM water_installation WHERE step = 'Phase-3-Step-1' AND (CONCAT(first_name, ' ', last_name, ' ', middle_name) LIKE '%$search_value%') ORDER BY time_updated DESC");

...
if (isset($search_value)) {
    ...
    if (is_string($search_value)) {
      $phase_4_step_1_proposed_tapping_sql = $conn->prepare("SELECT * FROM water_installation WHERE step = 'Phase-4-Step-1-Proposed-Tapping' AND (first_name LIKE '%$search_value%') OR (last_name LIKE '%$search_value%') OR (middle_name LIKE '%$search_value%') ORDER BY time_updated DESC");
